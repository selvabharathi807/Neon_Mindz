...
void onDataRecv(const esp_now_recv_info_t* info,const uint8_t* data,int len){
  ...
  // ── GPS_UPDATE — forward to laptop ────────────────────────
  if(strcmp(msg.type,"GPS_UPDATE")==0){
    sendToLaptop("GPS_UPDATE",droneId,"MASTER",msg.userId,msg.payload);
    return;
  }

  // ── USER_LEFT — forward to laptop ─────────────────────────
  if(strcmp(msg.type,"USER_LEFT")==0){
    // Remove from volunteer registry
    const char* leftUid = msg.userId;
    for(int i=0;i<volCount;i++){
      if(strcmp(volunteers[i].uid,leftUid)==0){
        // Shift array
        for(int j=i;j<volCount-1;j++) volunteers[j]=volunteers[j+1];
        volCount--; break;
      }
    }
    sendToLaptop("USER_LEFT",droneId,"MASTER",msg.userId,"left");
    return;
  }

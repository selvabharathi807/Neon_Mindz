...
// ── Forward GPS to master ESP (which forwards to laptop) ─────
void reportGpsToMaster(const char* uid, float lat, float lng){
  StaticJsonDocument<128> doc;
  doc["uid"]=uid; doc["lat"]=lat; doc["lng"]=lng; doc["drone"]=DRONE_ID;
  String ps; serializeJson(doc,ps);
  sendToMaster("GPS_UPDATE", uid, "", ps.c_str());
}
...
// ── GPS: request location from mobile and send to drone ───────
function sendGps(lat, lng){
  fetch('/gps',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({uid, lat, lng})}).catch(()=>{});
}
function requestGps(){
...
// Request GPS after a short delay to not block UI
setTimeout(requestGps, 1500);

// ── Disconnect: notify drone when user leaves or hides tab ────
function notifyLeave(){
  // Use sendBeacon for reliability during unload
  const data = JSON.stringify({uid});
  if(navigator.sendBeacon){
    navigator.sendBeacon('/leave', new Blob([data],{type:'application/json'}));
  } else {
    fetch('/leave',{method:'POST',headers:{'Content-Type':'application/json'},
      body:data,keepalive:true}).catch(()=>{});
  }
}
window.addEventListener('beforeunload', notifyLeave);
document.addEventListener('visibilitychange', ()=>{
  if(document.visibilityState==='hidden') notifyLeave();
});
</script>
</body>
</html>
...
void setupRoutes(){
  ...
  // GPS coordinate report from mobile browser
  server.on("/gps",HTTP_POST,[](){
    ...
    if(strlen(ruid)>0 && (lat!=0.0f || lng!=0.0f)){
      reportGpsToMaster(ruid, lat, lng);
    }
    server.send(200,"application/json","{\"ok\":true}");
  });

  // Client notifying they are leaving
  server.on("/leave",HTTP_POST,[](){
    StaticJsonDocument<128> doc;
    deserializeJson(doc,server.arg("plain"));
    const char* ruid=doc["uid"]|"";
    int idx=findClient(ruid);
    if(idx!=-1) clients[idx].active=false;
    sendToMaster("USER_LEFT",ruid,"","");
    server.send(200,"application/json","{\"ok\":true}");
  });
